    ...
    for embedding in ["clip","ssl","siglip2","dino"]:
        for data in ["league_captioned_splash"]:
            for n in [1000]:
                ...
                command=f"sbatch -J scale --err=slurm/scale/{name}.err --out=slurm/scale/{name}.out runpygpu_chip.sh "
                ...
                command+=f" --dataset jlbaker361/{embedding}-{data}-{n} --lr {lr} --embedding {embedding}  --use_lora "
                print(command)