    ...
    for start in [0]:
        ...
        for version in ["style", "style2" ]: #,"content","baseline","both"]:
            ...
            for epochs in [250,100]:
                for reward_fn in ["dino"]:
                    for facet in ["query","key","value","token"]:
                        name=f"{version}_{start}_{lr}_{epochs}_{reward_fn}_{facet}_prompt"
                        command=f"sbatch -J style --err=slurm/style_dino/{name}.err --out=slurm/style_dino/{name}.out"
                        command+=f" {exclude} runpygpu.sh main.py --start {start} --limit {limit} --reward_fn {reward_fn} --epochs {epochs} {flags}"
                        command+=f" --content_limit 1 --prompt_alignment --facet {facet} --project_name style-dino "
                        print(command)