    ...
    for unet in ["no-unet","unet"]:
        for pplus in ["no-pplus","pplus"]:
            for num_vectors in [1,3]:
                ...
                command=f"sbatch -J ir-ti-city --err=slurm/ir_city/{name}.err --out=slurm/ir_city/{name}.out"
                command+=f" {exclude} runpygpu.sh main_ir.py  --epochs 10000  {unet_flag} {pplus_flag} --num_vectors {num_vectors}"
                command+=f"   --project_name ti-ir-city --textual_inversion --prompt_src_txt famous_cities.txt"
