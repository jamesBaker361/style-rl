...
for lr in [0.001,0.0001]:
    for start in [0]:
        limit=start+4
        for version in ["baseline","style","style2" ]: #,"content","baseline","both"]:
            flags={
            ...
            for epochs in [250,100]:
                for reward_fn in ["ir"]:
                    ...
                    command=f"sbatch -J style --err=slurm/style_ir/{name}.err --out=slurm/style_ir/{name}.out"
                    command+=f" {exclude} runpygpu.sh main.py --start {start} --limit {limit} --reward_fn {reward_fn} --epochs {epochs} {flags}"
                    command+=f" --content_limit 1 --prompt_alignment --project_name rl-ir "
                    print(command)