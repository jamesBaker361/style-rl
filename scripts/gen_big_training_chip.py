port=29601

for training_type in  ["denoise","reward"]:
    for prediction_type in ["epsilon"]:
        for embedding in ["clip"]:
            for data in ["art_coco_captioned"]:
                name=f"{training_type}_{prediction_type}_{embedding}"
                port+=1
                command=f"sbatch  --gres=gpu:1  -J pers  --err=slurm_chip/pers_{data}/{name}.err --out=slurm_chip/pers_{data}/{name}.out"
                command+=f" runaccgpu_chip.sh  --mixed_precision fp16 --num_processes 1 --main_process_port {port} main_pers.py --epochs 4000 --limit -1 --batch_size 2 --project_name {data}-20 "
                command+=f" --mixed_precision fp16 --prediction_type {prediction_type} --upload_interval 100 "
                command+=f" --embedding {embedding} --training_type {training_type} --dataset jlbaker361/{embedding}-{data}-50 --vanilla --name jlbaker361/{name} --gradient_accumulation_steps 8 "
                print(command)