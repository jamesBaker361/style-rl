    ...
    for prediction_type in ["epsilon"]:
        for embedding in ["dino","ssl","clip","siglip2"]:
            for data in ["league_captioned_tile"]:
                ...
                command+=f" runaccgpu.sh --multi_gpu --mixed_precision fp16 --num_processes 2 --main_process_port {port} main_pers.py --epochs 250 --limit -1 --batch_size 4 --project_name {data}-20 "
