#copied from https://github.com/Tsingularity/dift/blob/main/src/models/dift_sd.py
...
class SDFeaturizer4Eval(SDFeaturizer):
    ...
    def forward(self,
        ...
        img_tensor = img_tensor.unsqueeze(0).repeat(ensemble_size, 1, 1, 1).to(device) # ensem, c, h, w
        if category in self.cat2prompt_embeds:
            prompt_embeds = self.cat2prompt_embeds[category]
        else:
            prompt_embeds = self.null_prompt_embeds
        prompt_embeds = prompt_embeds.repeat(ensemble_size, 1, 1).to(device)
        unet_ft_all = self.pipe(
            img_tensor=img_tensor,
            t=t,
            up_ft_indices=[up_ft_index],
