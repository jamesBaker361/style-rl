...
for training_type in  ["denoise","reward"]:
    for prediction_type in ["epsilon"]:
        for embedding in ["dino","clip","ssl","siglip2"]:
            for data in ["art_coco_captioned"]:
                ...
                command+=f" runaccgpu_chip.sh  --mixed_precision fp16 --num_processes 1 --main_process_port {port} main_pers.py --epochs 250 --limit -1 --batch_size 4 --project_name {data}-20 "
