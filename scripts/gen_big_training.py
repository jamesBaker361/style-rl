            ...
            for embedding in ["clip"]:
                for data in ["league_captioned_splash"]:
                    for lr in [0.001,0.0001]:
                        ...
                        command+=f" runaccgpu.sh --multi_gpu --mixed_precision fp16 --num_processes 2 --main_process_port {port} main_pers.py --epochs 400 --limit -1 --batch_size 2 --project_name {data}-500 "
                        ...
                        command+=f" --embedding {embedding} --training_type {training_type} --dataset jlbaker361/{embedding}-{data}-500 --vanilla --name jlbaker361/{name} --gradient_accumulation_steps 4  "
                        print(command)