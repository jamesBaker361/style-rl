            ...
            for data in ["art_coco_captioned"]:
                for frac in [1,0]:
                    for lr in [0.001,0.0001]:
                        ...
                        command+=f" runaccgpu_chip.sh  --mixed_precision fp16 --num_processes 1 --main_process_port {port} main_pers.py --epochs 5000 --limit -1 --batch_size 2 --project_name {data}-500 "
                        ...
                        command+=f" --embedding {embedding} --training_type {training_type} --dataset jlbaker361/{embedding}-{data}-500 --vanilla --name jlbaker361/{name} --gradient_accumulation_steps 8 "
                        print(command)