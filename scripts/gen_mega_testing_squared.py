                            ...
                            for n in [1000]:
                                ...
                                for pipeline in ["lcm"]:
                                    for reward_switch_epoch in [400]:
                                        ...
                                        port+=1
                                        for flag in ["increasing_scale","decreasing_scale","constant_scale"]:
                                            ...
                                            command+=f" runaccgpu.sh  --mixed_precision fp16 --num_processes 1 --main_process_port {port} main_logging.py --epochs 500 --limit 10 --batch_size 2 --project_name {data}-{n}-testing "
                                            command+=f" --mixed_precision fp16 --prediction_type {prediction_type} --upload_interval 1 --uncaptioned_frac {frac} --train_split 0.5 --lr {lr} --load --generic_test_prompts "
                                            ...
                                            command+=f" --pipeline {pipeline} --reward_switch_epoch {reward_switch_epoch}   --real_test_prompts --{flag} --num_inference_steps 8 "
