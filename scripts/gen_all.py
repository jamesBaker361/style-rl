for start in [0]:
    limit=start+4
    for version in ["style2" ,"style3"]: #"content","baseline","both"]:
        flags={
        ...
        for lr in [0.001,0.0001,0.00001]:
            ...
            for reward in ["vgg"]:
                for method in ["hook","ddpo","align"]:
                    ...
                    command=f" sbatch -J style {exclude} --err=slurm/style/{name}.err --out=slurm/style/{name}.out runpygpu.sh main.py --batch_size 1 --project_name style-rl "
