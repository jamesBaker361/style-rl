...
for lr in [0.001,0.01]:
    for reward_fn in ["qualiclip","ir","dino"]:
        for prompt_src in ["animals","nature"]:
            for method in ["unet", "pp","ti"]:
                for submethod in ["vanilla","nemesis"]:
                    ...
                    command=f"sbatch -J {reward_fn}-{prompt_src}-ti --err=slurm/{prompt_src}/{name}.err --out=slurm/{prompt_src}/{name}.out"
                    command+=f" {exclude} runpygpu.sh main_ir.py  --epochs 100  {method_flags} {submethod_flags} --num_vectors 1 "
                    command+=f"  --textual_inversion --prompt_src_txt {src_text} "
                    command+=f" --validation_epochs 40 --learning_rate {lr} --reward_fn {reward_fn} "
                    command+=f" --project_name testing --per_prompt_stat_tracking_buffer_size 3 "
                    #command+=f"  --project_name {reward_fn}_{prompt_src}"
                    print(command)
