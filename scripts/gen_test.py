    ...
    for prediction_type in ["epsilon","v_prediction"]:
        for embedding in ["clip"]:
            ...
            for pipeline in ["sana"]:
                name=f"{training_type}_{prediction_type}_{embedding}_{pipeline}"
                ...
                command+=f" --mixed_precision fp16 --prediction_type {prediction_type} --pipeline {pipeline} "
                command+=f" --embedding {embedding} --training_type {training_type} --dataset jlbaker361/{embedding}-art_coco_captioned-50-{pipeline}"
                print(command)