                            ...
                            for n in [1000]:
                                for scheduler in ["LCMScheduler"]:
                                    for pipeline in ["lcm","lcm_pre_lora","lcm_post_lora"]:
                                        ...
                                        name=f"{training_type}_{prediction_type}_{embedding}_{frac}_{lr}_{n}{suffix}_{scheduler}_{pipeline}"
                                        port+=1
                                        command=f"sbatch  -J pers  --err=slurm/pers_{data}/{name}.err --out=slurm/pers_{data}/{name}.out"
                                        command+=f" runaccgpu.sh --multi_gpu --mixed_precision fp16 --num_processes 2 --main_process_port {port} main_pers.py --epochs 500 --limit -1 --batch_size 2 --project_name {data}-{n} "
                                        ...
                                        command+=f" --scheduler_type {scheduler} "
                                        if suffix=="_no_proj":
