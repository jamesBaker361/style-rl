...
def main(args):
    ...
    def logging(data_loader,pipeline,baseline:bool=False,auto_log:bool=True,clip_model:CLIPModel=clip_model):
        ...
        start=time.time()
        fid.update(torch.cat(image_list).to(fid.inception.feature_extractor_internal_dtype),real=True)
        fid.update(torch.cat(fake_image_list).to(fid.inception.feature_extractor_internal_dtype),real=False)
