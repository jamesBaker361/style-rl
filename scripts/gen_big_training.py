    ...
    for prediction_type in ["epsilon"]:
        for embedding in ["dino","ssl","clip","siglip2"]:
            for data in ["league_captioned_tile"]:
                ...
                command+=f" --embedding {embedding} --training_type {training_type} --dataset jlbaker361/{embedding}-{data} --vanilla --name jlbaker361/{name} --gradient_accumulation_steps 8 "
                print(command)