            ...
            for step_set in ["mid","late","early"]:
                for limited in ["yes","no"]:
                    for data in ["sshq-100","mtg"]:
                        ...
                        command=f"sbatch -J seg  --err=slurm_chip/overlap/_{name}.err --out=slurm_chip/overlap/_{name}.out runpygpu_chip.sh main_seg.py --segmentation_attention_method overlap --initial_mask_step_list 1 2 --final_mask_steps_list {final_mask_steps_list} --final_adapter_steps_list {final_adapter_steps_list} --limit 32 --overlap_frac {overlap_frac} "
