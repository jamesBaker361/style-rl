                    ...
                    for suffix in ["_identity"]:
                        for lr in [0.001]:
                            for n in [1000]:
                                ...
                                for pipeline in ["lcm"]:
                                    for reward_switch_epoch in [-1,400]:
                                        ...
                                        command+=f" runaccgpu.sh  --mixed_precision fp16 --num_processes 1 --main_process_port {port} main_pers.py --epochs 500 --limit -1 --batch_size 2 --project_name {data}-{n}-beta "
                                        ...
                                        command+=f" --pipeline {pipeline} --reward_switch_epoch {reward_switch_epoch} "
