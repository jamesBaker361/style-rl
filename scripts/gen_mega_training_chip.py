                            ...
                            for n in [1000]:
                                ...
                                for pipeline in ["lcm"]:
                                    for reward_switch_epoch in [-1]:
                                        ...
                                        port+=1
                                        command=f"sbatch  -J pers  --err=slurm_chip/pers2_{data}/{name}.err --out=slurm_chip/pers2_{data}/{name}.out --gres=gpu:1 "
                                        command+=f" runpygpu_chip.sh  --mixed_precision fp16 --num_processes 1 --main_process_port {port} main_pers.py --epochs 1000 --limit -1 --batch_size 2 --project_name {data}-{n}-beta "
