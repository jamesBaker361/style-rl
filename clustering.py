import os
import argparse
from experiment_helpers.gpu_details import print_details
from accelerate import Accelerator
import time
import numpy as np
from sklearn.svm import LinearSVC
from sklearn.linear_model import SGDClassifier
from sklearn.utils.class_weight import compute_class_weight
from datasets import load_dataset,Dataset
from sklearn.preprocessing import StandardScaler
import joblib  # for saving

parser=argparse.ArgumentParser()

parser.add_argument("--project_name",type=str,default="evaluation-creative")
parser.add_argument("--src_dataset",type=str,default="jlbaker361/league-tagged-clip")
parser.add_argument("--dest_dataset",type=str,default="jlbaker361/league-clip-clustering")



def main(args):
    accelerator=Accelerator(log_with="wandb")
    
    data=load_dataset(args.src_dataset,split="train")
    all_tags=set(data["tag"])
    all_champions=set(data["champion"])

    plane_dict={
        "cluster_embedding":[],
        "type":[], #tag or character
        "label":[], #the tag or character in question
        "positives":[] #how many positive examples there were
    }
    X=[row["embedding"][0] for row in data]

    
    for tag in all_tags:

        
        tag_data=data.filter(lambda row: row["tag"]==tag)
        cluster_embedding=np.mean(tag_data["embedding"])
        plane_dict["cluster_embedding"].append(cluster_embedding)
        plane_dict["type"].append("tag")
        plane_dict["label"].append(tag)
        plane_dict["positives"].append(len(tag_data))

    for champion in all_champions:

        
        champion_data=data.filter(lambda row: row["champion"]==champion)
        cluster_embedding=np.mean(champion_data["embedding"])
        plane_dict["cluster_embedding"].append(cluster_embedding)
        plane_dict["type"].append("champion")
        plane_dict["label"].append(champion)
        plane_dict["positives"].append(len(champion_data))
    '''print("Hyperplane: w·x + b = 0")
    print("w =", w)
    print("b =", b)'''
    Dataset.from_dict(plane_dict).push_to_hub(args.dest_dataset)


if __name__=='__main__':
    print_details()
    start=time.time()
    args=parser.parse_args()
    print(args)
    main(args)
    end=time.time()
    seconds=end-start
    hours=seconds/(60*60)
    print(f"successful generating:) time elapsed: {seconds} seconds = {hours} hours")
    print("all done!")