        ...
        for prediction_type in ["epsilon"]:
            for embedding in ["clip"]:
                for data in ["league_captioned_splash"]:
                    ...
                    command+=f" runaccgpu.sh --multi_gpu --mixed_precision fp16 --num_processes 2 --main_process_port {port} main_pers.py --epochs 8000 --limit -1 --batch_size 2 --project_name {data}-50-{frac} "
                    command+=f" --mixed_precision fp16 --prediction_type {prediction_type} --upload_interval 100 --uncaptioned_frac {frac} "
