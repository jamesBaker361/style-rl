    ...
    for unet in ["no-unet","unet"]:
        for pplus in ["no-pplus","pplus"]:
            for num_vectors in [1,3]:
                ...
                command+=f" {exclude} runpygpu.sh main.py  --epochs 1000  {unet_flag} {pplus_flag} --num_vectors {num_vectors}"
                command+=f"  --prompt_alignment --project_name ti-ir --textual_inversion --prompt_src_txt random_animals.txt"
